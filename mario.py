...
class Mario(Sprite):
    ...
    def __init__(self, screen, settings, pipes, bricks, upgrades, stats, enemies, poles, music, sounds, fireballs, secret_bricks, secret_pipes, ground):
        ...
        spriteSheet = pygame.image.load('resources/allsprites.png')
        self.image.set_colorkey((0, 0, 0))
        self.image.blit(spriteSheet, (0, 0), (59, 0, 17, 16))
        ...
        for i in range(0, 13):
            ...
            temp_img.blit(spriteSheet, (0, 0), (59, i * 20, 17, 16))
            temp = pygame.transform.scale(temp_img, (40, 40))
            self.small_mario.append(temp)
            temp_img = pygame.Surface((17, 16))
            temp_img.set_colorkey((0, 0, 0))
            temp_img.blit(spriteSheet, (0, 0), (80, i * 20, 17, 16))
            temp = pygame.transform.scale(temp_img, (40, 40))
            self.small_star_mario.append(temp)
            temp_img = pygame.Surface((17, 16))
            temp_img.set_colorkey((0, 0, 0))
            temp_img.blit(spriteSheet, (0, 0), (100, i * 20, 17, 16))
            temp = pygame.transform.scale(temp_img, (40, 40))
            self.small_star_mario.append(temp)
            temp_img = pygame.Surface((17, 32))
            temp_img.set_colorkey((0, 0, 0))
            temp_img.blit(spriteSheet, (0, 0), (120, i * 40, 17, 32))
            temp = pygame.transform.scale(temp_img, (40, 60))
            self.shroom_mario.append(temp)
            temp_img = pygame.Surface((17, 32))
            temp_img.set_colorkey((0, 0, 0))
            temp_img.blit(spriteSheet, (0, 0), (140, i * 40, 17, 32))
            temp = pygame.transform.scale(temp_img, (40, 60))
            self.flower_mario.append(temp)
            self.star_mario.append(temp)
            temp_img = pygame.Surface((17, 32))
            temp_img.set_colorkey((0, 0, 0))
            temp_img.blit(spriteSheet, (0, 0), (160, i * 40, 17, 32))
            temp = pygame.transform.scale(temp_img, (40, 60))
            self.star_mario.append(temp)
        temp_img = pygame.Surface((17, 16))
        temp_img.set_colorkey((0, 0, 0))
        temp_img.blit(spriteSheet, (0, 0), (59, 260, 17, 16))
        temp = pygame.transform.scale(temp_img, (40, 40))
        self.small_mario.append(temp)
    ...
    def update(self, stats, level, sounds):
        self.invincible()
        if self.dead:
            self.image = self.small_mario[12]
            self.die_animate(stats, level, sounds)
        else:
        ...
        if pygame.sprite.spritecollide(self, self.poles, False):
            ...
            if not self.stats.secret_level:
                ...
                for brick in brick_collide:
                    ...
                    if brick.rect.x - 20 < self.rect.x < brick.rect.x + 20 and brick.rect.y < self.rect.y and brick.block_type == 5 and not brick.change_brick:
                    ...
                    if brick.rect.x - 20 < self.rect.x < brick.rect.x + 20 and brick.rect.y < self.rect.y and brick.block_type == 6 and not brick.change_brick:
                        brick.change_brick = True
                        upgrade = Upgrade(self.screen, self.settings, self.pipes, self.bricks, brick.rect.x, brick.rect.y - 20, 2)
                        self.upgrades.add(upgrade)
                        brick.change()

                    if brick.rect.x - 20 < self.rect.x < brick.rect.x + 20 and brick.rect.y < self.rect.y and brick.block_type == 2:
                        brick.change()
                        self.sounds[3].play()
                        if brick.block_type == 2 and not brick.change_brick and brick.rect.y < self.rect.y and not self.shroomed:
                            brick.change_brick = True
                            upgrade = Upgrade(self.screen, self.settings, self.pipes, self.bricks, brick.rect.x, brick.rect.y - 20, 0)
                            self.upgrades.add(upgrade)
                        if brick.block_type == 2 and not brick.change_brick and brick.rect.y < self.rect.y and self.shroomed:
                            brick.change_brick = True
                            upgrade = Upgrade(self.screen, self.settings, self.pipes, self.bricks, brick.rect.x, brick.rect.y - 40, 1)
                            self.upgrades.add(upgrade)
                    if brick.rect.x - 20 < self.rect.x < brick.rect.x + 20 and brick.rect.y < self.rect.y and brick.block_type == 1:
                        brick.change()
                        self.sounds[3].play()
                        self.stats.coins += 1
                        self.stats.score += 100

                    if brick.rect.x - 20 < self.rect.x < brick.rect.x + 20 and brick.rect.y < self.rect.y and brick.block_type == 9 and not brick.change_brick:
                        if self.count != 4:
                            self.stats.coins += 1
                            self.count += 1
                        else:
                    ...
                    if brick.rect.x - 20 < self.rect.x < brick.rect.x + 20 and brick.rect.y < self.rect.y and brick.block_type == 0 and self.shroomed:
                        self.bricks.remove(brick)
                        self.sounds[2].play()
                    elif brick.rect.x - 20 < self.rect.x < brick.rect.x + 20 and brick.rect.y < self.rect.y and brick.block_type == 0 and not self.shroomed:
                        brick.bouncing = True
                        self.sounds[1].play()
    ...
    def die_animate(self, stats, level, sounds):
        self.image = self.small_mario[12]
    ...
    def reset_mario(self, level, sounds):
        self.enemies.empty()
        self.bricks.empty()
